...
def resolve_jsbsim_data_dir():
    root_dir = os.path.normpath(get_root_dir())
    candidate_dirs = [
        os.path.join(root_dir, 'data'),
        os.path.join(root_dir, '.jsbsim_data_tmp'),
    ]
    required_file = os.path.join('aircraft', 'f16', 'f16.xml')

    for candidate_dir in candidate_dirs:
        if os.path.exists(os.path.join(candidate_dir, required_file)):
            return candidate_dir

    raise FileNotFoundError(
        "JSBSim data directory is missing required aircraft files. "
        f"Checked: {candidate_dirs}. Expected file: {required_file}"
    )
