...
class SingleCombatTask(BaseTask):
    def __init__(self, config):
        ...
        self.artillery_damage_scale = getattr(self.config, 'artillery_damage_scale', 1.0)
        self.artillery_close_range_km = getattr(self.config, 'artillery_close_range_km', 1.0)
        self.artillery_far_range_km = getattr(self.config, 'artillery_far_range_km', 3.0)
        self.artillery_max_attack_angle_rad = np.deg2rad(getattr(self.config, 'artillery_max_attack_angle_deg', 30.0))
        self.use_low_altitude_safety_shield = getattr(self.config, 'use_low_altitude_safety_shield', False)
        self.low_altitude_safety_floor_m = getattr(
            self.config,
            'low_altitude_safety_floor_m',
            float(getattr(self.config, 'altitude_limit', 2500)) + 900.0,
        )
        self.low_altitude_safety_recovery_band_m = getattr(
            self.config,
            'low_altitude_safety_recovery_band_m',
            1800.0,
        )
        self.low_altitude_safety_max_sink_rate_mps = getattr(
            self.config,
            'low_altitude_safety_max_sink_rate_mps',
            45.0,
        )
        self.low_altitude_safety_pullup_elevator_norm = getattr(
            self.config,
            'low_altitude_safety_pullup_elevator_norm',
            -0.65,
        )
        self.low_altitude_safety_level_elevator_norm = getattr(
            self.config,
            'low_altitude_safety_level_elevator_norm',
            -0.10,
        )
        self.low_altitude_safety_max_aileron_norm = getattr(
            self.config,
            'low_altitude_safety_max_aileron_norm',
            0.20,
        )
        self.low_altitude_safety_max_rudder_norm = getattr(
            self.config,
            'low_altitude_safety_max_rudder_norm',
            0.18,
        )
        self.low_altitude_safety_min_throttle_norm = getattr(
            self.config,
            'low_altitude_safety_min_throttle_norm',
            0.78,
        )
        self._safety_shield_interventions = {}
        self._safety_shield_steps = {}
    ...
    def normalize_action(self, env, agent_id, action):
        ...
        if self.use_baseline and agent_id in env.enm_ids:
            ...
            return self._apply_low_altitude_safety_shield(env, agent_id, np.asarray(action, dtype=np.float32))
        else:
    ...
    @staticmethod
    def _looks_like_continuous_action(action):
        action = np.asarray(action, dtype=np.float32).reshape(-1)
        if action.shape[0] != 4:
            return False
        return (
            np.all(action[:3] >= -1.05)
            and np.all(action[:3] <= 1.05)
            and 0.35 <= float(action[3]) <= 0.95
        )
    ...
    def reset(self, env):
        ...
        self._safety_shield_interventions = {agent_id: 0 for agent_id in env.agents.keys()}
        self._safety_shield_steps = {agent_id: 0 for agent_id in env.agents.keys()}
        if self.use_baseline:
            self.baseline_agent.reset()
        return super().reset(env)

    def _compute_low_altitude_safety_strength(self, env, agent_id):
        if not self.use_low_altitude_safety_shield:
            return 0.0
        aircraft = env.agents[agent_id]
        if not aircraft.is_alive:
            return 0.0
        altitude = aircraft.get_property_value(c.position_h_sl_m)
        sink_rate = max(float(aircraft.get_property_value(c.velocities_v_down_mps)), 0.0)
        pitch = float(aircraft.get_property_value(c.attitude_pitch_rad))
        band_top = self.low_altitude_safety_floor_m + self.low_altitude_safety_recovery_band_m
        altitude_risk = np.clip((band_top - altitude) / max(self.low_altitude_safety_recovery_band_m, 1e-6), 0.0, 1.0)
        sink_risk = np.clip(sink_rate / max(self.low_altitude_safety_max_sink_rate_mps, 1e-6), 0.0, 1.0)
        pitch_risk = np.clip((-pitch) / np.deg2rad(18.0), 0.0, 1.0)
        return float(np.clip(0.60 * altitude_risk + 0.25 * sink_risk + 0.15 * pitch_risk, 0.0, 1.0))

    def _apply_low_altitude_safety_shield(self, env, agent_id, norm_act):
        norm_act = np.asarray(norm_act, dtype=np.float32).copy()
        if agent_id not in self._safety_shield_steps:
            self._safety_shield_steps[agent_id] = 0
            self._safety_shield_interventions[agent_id] = 0
        self._safety_shield_steps[agent_id] += 1

        strength = self._compute_low_altitude_safety_strength(env, agent_id)
        if strength <= 0.0:
            return norm_act

        original = norm_act.copy()
        elevator_cap = (
            (1.0 - strength) * self.low_altitude_safety_level_elevator_norm
            + strength * self.low_altitude_safety_pullup_elevator_norm
        )
        aileron_cap = (1.0 - strength) * 1.0 + strength * self.low_altitude_safety_max_aileron_norm
        rudder_cap = (1.0 - strength) * 1.0 + strength * self.low_altitude_safety_max_rudder_norm
        throttle_floor = (1.0 - strength) * 0.4 + strength * self.low_altitude_safety_min_throttle_norm

        norm_act[0] = np.clip(norm_act[0], -aileron_cap, aileron_cap)
        norm_act[1] = min(norm_act[1], elevator_cap)
        norm_act[2] = np.clip(norm_act[2], -rudder_cap, rudder_cap)
        norm_act[3] = max(norm_act[3], throttle_floor)

        if np.max(np.abs(norm_act - original)) > 1e-6:
            self._safety_shield_interventions[agent_id] += 1
        return norm_act

    def get_safety_shield_rate(self, team_ids):
        steps = sum(self._safety_shield_steps.get(agent_id, 0) for agent_id in team_ids)
        if steps <= 0:
            return 0.0
        interventions = sum(self._safety_shield_interventions.get(agent_id, 0) for agent_id in team_ids)
        return float(interventions / steps)

    def step(self, env):
        def _orientation_fn(AO):
            if AO >= 0 and AO <= self.artillery_max_attack_angle_rad:
                return 1 - AO / max(self.artillery_max_attack_angle_rad, 1e-6)
            elif AO >= -self.artillery_max_attack_angle_rad and AO <= 0:
                return 1 + AO / max(self.artillery_max_attack_angle_rad, 1e-6)
            return 0
        def _distance_fn(R):
            if R <= self.artillery_close_range_km:
                return 1
            elif R > self.artillery_close_range_km and R <= self.artillery_far_range_km:
                return (self.artillery_far_range_km - R) / max(self.artillery_far_range_km - self.artillery_close_range_km, 1e-6)
            else:
                return 0
            ...
            for agent_id in env.agents.keys():
                ...
                for enm in env.agents[agent_id].enemies:
                    if enm.is_alive:
                        ...
                        enm.bloods -= self.artillery_damage_scale * _orientation_fn(AO) * _distance_fn(R/1000)
