...
from .monitor import JSONLMetricsWriter, build_progress_bar, extract_episode_summaries, summarize_episode_summaries
...
class ShareJSBSimRunner(Runner):
    @staticmethod
    def _safe_average_episode_rewards(buffer):
        completed = int((buffer.masks == False).sum())
        if completed > 0:
            return float(buffer.rewards.sum() / completed)
        return float(buffer.rewards.mean())
    ...
    def load(self):
        ...
        self.use_independent_critic = self.algorithm_name == "ippo"
        ...
        critic_obs_space = self.obs_space if self.use_independent_critic else self.share_obs_space
        self.policy = Policy(self.all_args, self.obs_space, critic_obs_space, self.act_space, device=self.device)
        self.trainer = Trainer(self.all_args, device=self.device)

        # buffer
        if self.use_selfplay:
            share_buffer_space = self.obs_space if self.use_independent_critic else self.share_obs_space
            self.buffer = SharedReplayBuffer(self.all_args, self.num_agents // 2, self.obs_space, share_buffer_space, self.act_space)
        else:
            share_buffer_space = self.obs_space if self.use_independent_critic else self.share_obs_space
            self.buffer = SharedReplayBuffer(self.all_args, self.num_agents, self.obs_space, share_buffer_space, self.act_space)
        ...
        if self.use_selfplay:
            ...
            self.opponent_policy = [
                Policy(self.all_args, self.obs_space, critic_obs_space, self.act_space, device=self.device)
            ...
            if self.use_eval:
                self.eval_opponent_policy = Policy(self.all_args, self.obs_space, critic_obs_space, self.act_space, device=self.device)
    ...
    def _set_policy_progress(self, progress):
        self.policy.set_progress(progress)
        if self.use_selfplay:
            for opponent_policy in self.opponent_policy:
                opponent_policy.set_progress(progress)
            if self.use_eval:
                self.eval_opponent_policy.set_progress(progress)

    def _record_episode_summaries(self, infos):
        episode_summaries = extract_episode_summaries(infos)
        self.recent_episode_summaries.extend(episode_summaries)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            self._set_policy_progress(progress)
            ...
            if episode % self.log_interval == 0:
                ...
                train_infos["average_episode_rewards"] = self._safe_average_episode_rewards(self.buffer)
                recent_episode_metrics = summarize_episode_summaries(list(self.recent_episode_summaries))
                train_infos.update(recent_episode_metrics)
                train_infos["progress"] = progress
                train_infos["progressive_stage"] = getattr(self.policy.actor.act, "current_progressive_stage", 0)
                progress_bar = build_progress_bar(progress, self.progress_bar_width)
                logging.info(
                    "{} ep {}/{} | stage:{} | reward:{:.3f} | win:{:.3f} loss:{:.3f} draw:{:.3f} | "
                    "end[low:{:.3f} dist:{:.3f} timeout:{:.3f}] | shield:{:.3f} | steps:{:.1f}".format(
                        progress_bar,
                        episode + 1,
                        episodes,
                        train_infos["progressive_stage"],
                        float(train_infos["average_episode_rewards"]),
                        float(train_infos["win_rate"]),
                        float(train_infos["loss_rate"]),
                        float(train_infos["draw_rate"]),
                        float(train_infos["low_altitude_rate"]),
                        float(train_infos["distance_out_rate"]),
                        float(train_infos["timeout_rate"]),
                        float(train_infos["safety_shield_rate"]),
                        float(train_infos["avg_episode_steps"]),
                    )
                )
                ...
                self.metrics_writer.write({
                    ...
                    **{
                        key: float(value) if isinstance(value, (np.floating, np.integer, float, int)) else value
    ...
    def eval(self, total_num_steps):
        ...
        while total_episodes < self.eval_episodes:
            ...
            if self.use_independent_critic:
                eval_share_obs = eval_obs.copy()
            eval_episode_summaries.extend(extract_episode_summaries(eval_infos))
        ...
        eval_infos.update({f"eval_{key}": value for key, value in summarize_episode_summaries(eval_episode_summaries).items()})
        logging.info(
            " eval reward:{:.3f} | win:{:.3f} loss:{:.3f} draw:{:.3f} | end[low:{:.3f} dist:{:.3f} timeout:{:.3f}] | shield:{:.3f}".format(
                float(eval_infos['eval_average_episode_rewards']),
                float(eval_infos.get('eval_win_rate', 0.0)),
                float(eval_infos.get('eval_loss_rate', 0.0)),
                float(eval_infos.get('eval_draw_rate', 0.0)),
                float(eval_infos.get('eval_low_altitude_rate', 0.0)),
                float(eval_infos.get('eval_distance_out_rate', 0.0)),
                float(eval_infos.get('eval_timeout_rate', 0.0)),
                float(eval_infos.get('eval_safety_shield_rate', 0.0)),
            )
        )
        self.last_eval_metrics = eval_infos.copy()
