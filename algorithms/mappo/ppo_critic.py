...
class PPOCritic(nn.Module):
    def __init__(self, args, obs_space, local_obs_space=None, device=torch.device("cpu")):
        ...
        if self.use_attention_policy:
            local_obs_dim = 0
            if local_obs_space is not None and hasattr(local_obs_space, "shape") and len(local_obs_space.shape) > 0:
            ...
            self.base = MultiDimensionalAttentionBase(
                ...
                group_dim=local_obs_dim,
                query_token_count=args.attention_critic_query_tokens,
            )
        else:
