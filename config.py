...
def get_config():
    ...
    parser = _get_offpolicy_config(parser)
...
def _get_prepare_config(parser: argparse.ArgumentParser):
    ...
    Prepare parameters:
        ...
        --algorithm-name <str>
            specifiy the algorithm, including `["ppo", "mappo", "ippo", "matd3"]`
        --experiment-name <str>
    ...
    group.add_argument("--algorithm-name", type=str, default='ppo', choices=["ppo", "mappo", "ippo", "matd3"],
...
def _get_network_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--use-attention-policy", action='store_true', default=False,
                       help="Whether to replace the default MLP encoder with a multi-dimensional attention encoder")
    group.add_argument("--attention-embed-dim", type=int, default=128,
                       help="Embedding size used by the attention encoder")
    group.add_argument("--attention-num-heads", type=int, default=4,
                       help="Number of heads used by the attention encoder")
    group.add_argument("--attention-dropout", type=float, default=0.0,
                       help="Dropout used by the attention encoder")
    group.add_argument("--attention-sparse-topk", type=int, default=2,
                       help="Top-k tokens kept by the sparse attention branch (0 disables hard sparsification)")
    group.add_argument("--attention-critic-query-tokens", type=int, default=2,
                       help="Number of leading tokens used to build the critic cross-attention query")
    group.add_argument("--use-progressive-action-discretization", action='store_true', default=False,
                       help="Whether to mask the action space from coarse to fine during training")
    group.add_argument("--progressive-action-strides", type=str, default='10 10 10 7|5 5 5 4|2 2 2 2|1 1 1 1',
                       help="Per-stage action strides for progressive discretization, split stages with |")
    group.add_argument("--progressive-stage-boundaries", type=str, default='0.0 0.35 0.7 0.9',
                       help="Progress thresholds for each progressive discretization stage")
    return parser
...
def _get_optimizer_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--critic-lr", type=float, default=5e-4,
                       help='critic learning rate used by off-policy algorithms (default: 5e-4)')
    return parser


def _get_offpolicy_config(parser: argparse.ArgumentParser):
    """
    Off-policy actor-critic parameters:
        --offpolicy-buffer-capacity <int>
            replay buffer capacity for off-policy algorithms
        --batch-size <int>
            minibatch size used for off-policy updates
        --learning-starts <int>
            number of environment steps before gradient updates start
        --updates-per-step <int>
            number of gradient updates performed per environment step
        --tau <float>
            Polyak averaging factor for target networks
        --policy-noise <float>
            target policy smoothing noise std for TD3-style algorithms
        --noise-clip <float>
            noise clipping range for target policy smoothing
        --exploration-noise <float>
            std of Gaussian exploration noise applied to the actor output
        --policy-delay <int>
            delayed actor update interval
    """
    group = parser.add_argument_group("Off-policy parameters")
    group.add_argument("--offpolicy-buffer-capacity", type=int, default=200000,
                       help="Replay buffer capacity for off-policy algorithms")
    group.add_argument("--batch-size", type=int, default=256,
                       help="Minibatch size for off-policy updates")
    group.add_argument("--learning-starts", type=int, default=5000,
                       help="Number of environment steps collected before training starts")
    group.add_argument("--updates-per-step", type=int, default=1,
                       help="Gradient updates performed after each environment step")
    ...
    group.add_argument("--policy-noise", type=float, default=0.2,
                       help="Target policy smoothing noise used by TD3-style methods")
    group.add_argument("--noise-clip", type=float, default=0.5,
                       help="Clipping bound of target policy smoothing noise")
    group.add_argument("--exploration-noise", type=float, default=0.1,
                       help="Gaussian exploration noise added to the actor output")
    group.add_argument("--policy-delay", type=int, default=2,
                       help="Delayed actor update interval for TD3-style methods")
    return parser
...
def _get_log_config(parser: argparse.ArgumentParser):
    ...
    group.add_argument("--progress-bar-width", type=int, default=72,
                       help="Width of the textual progress bar used during training")
    group.add_argument("--episode-stats-window", type=int, default=128,
                       help="Window size used to smooth training outcome statistics")
    group.add_argument("--export-metrics", action='store_true', default=True,
                       help="Whether to export training metrics as jsonl records")
    group.add_argument("--metrics-filename", type=str, default='metrics.jsonl',
                       help="Filename used to export training metrics inside the run directory")
    return parser
